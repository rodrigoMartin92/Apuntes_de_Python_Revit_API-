    ...
    # 2 - Compilacion de metodos usados
...
import AutodeskSeleccion_de_prueba
from Autodesk.Revit.DB import *
from Autodesk.Revit.UI import *
from Autodesk.Revit.Exceptions import *        
...
# Agregar referencias a las APIs de Revit necesarias
clr.AddReference('RevitAPI')
from Autodesk.Revit.DB import *
...
def ConvertirUnidades(numero):
        try:
            UserInterface_Unidades = Document.GetUnits(doc).GetFormatOptions(SpecTypeId.Length).GetUnitTypeId()
            return UnitUtils.ConvertToInternalUnits(numero, UserInterface_Unidades)
        except Exception as e:
            print(f"Error en 'ConvertirUnidades': {e}")


def ColocarFamilia(NombreDeFamilia, Punto):
    try:
        collector_EF = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalFixtures).WhereElementIsElementType().ToElements()
        familia = next((i for i in collector_EF if i.Family.Name == NombreDeFamilia), None)
        
        if not familia:
            raise ValueError(f"Family '{NombreDeFamilia}' not found.")

        Nivel = doc.ActiveView.GenLevel
        
        TransactionManager.Instance.EnsureInTransaction(doc)
        
        if not familia.IsActive:
            familia.Activate()
        Instancia = doc.Create.NewFamilyInstance(Punto, familia, Nivel, StructuralType.NonStructural)

    except Exception as e:
        print(f"Error en 'ColocarFamilia': {e}")
        Instancia = None
...
def ColocarValorElectricoElectrico(TipoDeNombre, family):
    Relacion_ValorElectrico_Pulgadas = {
        '32"': 50,
        '40"': 60,
        '50"': 65,
        '55"': 70,
    ...
    ValorElectrico = Relacion_ValorElectrico_Pulgadas.get(TipoDeNombre)
    
    if ValorElectrico is None:
        raise ValueError(f"ValorElectrico for type '{TipoDeNombre}' not found.")
    
    PotencialElectrico = Document.GetUnits(doc).GetFormatOptions(SpecTypeId.ElectricalPotential).GetUnitTypeId()
    ValorElectrico_internal = UnitUtils.ConvertToInternalUnits(ValorElectrico, PotencialElectrico)
    
    TransactionManager.Instance.EnsureInTransaction(doc)
    family.LookupParameter('ValorElectrico').Set(ValorElectrico_internal)
    TransactionManager.Instance.ForceCloseTransaction()
    
    return ValorElectrico_internal

def GirarElemento(element, trelement, tr_name):
    rotation = element.FacingOrientation
    PuntoDeOrigen = trelement.Location.Point
    rAxis = Line.CreateBound(PuntoDeOrigen, XYZ(PuntoDeOrigen.X, PuntoDeOrigen.Y, PuntoDeOrigen.Z + 1))
    angle = rotation.AngleTo(XYZ(0, 1, 0))
    
    TransactionManager.Instance.EnsureInTransaction(doc)
    ElementTransformUtils.RotateElement(doc, trelement.Id, rAxis, -angle)
    TransactionManager.Instance.TransactionTaskDone()

def process_tv_elements(doc, LinkDocumento, search):
    collector_CD = FilteredElementCollector(LinkDocumento).OfCategory(BuiltInCategory.OST_CommunicationDevices).WhereElementIsNotElementType().ToElements()
    
    NombresTV = []
    ValorElectricoesElectricidad = []

    for elemento in collector_CD:
        NombreDeFamilia = elemento.get_Symbol().FamilyName
        Punto_1 = elemento.Location.Point
        
        if NombreDeFamilia == 'TV_wall':
            AlturaTV = elemento.get_Symbol().LookupParameter('alto').AsDouble()
            Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z - (AlturaTV / 2 + ConvertirUnidades(100)))
            FamiliaColocada = ColocarFamilia('TR_wall', Punto2)
            if family:
                GirarElemento(elemento, FamiliaColocada, 'TR_vwall')
                _Electricidad = ColocarValorElectricoElectrico(elemento.Name, FamiliaColocada)
                NombresTV.append(element.Name)
                ValorElectricoesElectricidad.append(FamiliaColocada.LookupParameter('ValorElectrico').AsValueString())
        
        elif NombreDeFamilia == 'TV_ceiling':
            AlturaSoporteTV = elemento.LookupParameter('alto_soporte').AsDouble()
            Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z + AlturaSoporteTV)
            FamiliaColocada = ColocarFamilia('TR_ceiling', Punto2)
            if FamiliaColocada:
                GirarElemento(elemento, FamiliaColocada, 'TR_ceiling')
                _Electricidad = ColocarValorElectricoElectrico(elemento.Name, FamiliaColocada)
                NombresTV.append(elemento.Name)
                ValorElectricoesElectricidad.append(FamiliaColocada.LookupParameter('ValorElectrico').AsValueString())
        
        else:
            print("No existe las familias del tipo TV_wall o TV_ceiling")

    return list(zip(NombresTV, ValorElectricoesElectricidad))

doc = DocumentManager.Instance.CurrentDBDocument

# Documentos vinculados
DocumentoSeleccionado = FilteredElementCollector(doc).OfClass(RevitLinkInstance).ToElements()
search = 'modeloAV.rvt'

# Procesar documentos vinculados
for linked in DocumentoSeleccionado:
    LinkDocumento = linked.GetLinkDocument()
    if search in linked.Name:
        output = process_tv_elements(doc, LinkDocumento, search)
        break
...
def process_tv_elements(doc, LinkDocumento, search):
    ...
    for elemento in collector_CD:
        ...
        if NombreDeFamilia == 'TV_wall':
            AlturaTV = elemento.get_Symbol().LookupParameter('alto').AsDouble()
            Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z - (AlturaTV / 2 + UnitUtils.ConvertToInternalUnits(numero, Document.GetUnits(doc).GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())(100)))
            FamiliaColocada = ColocarFamilia('TR_wall', Punto2)
            if FamiliaColocada:
                GirarElemento(elemento, FamiliaColocada, 'TR_vwall')
                _Electricidad = ColocarValorElectricoElectrico(elemento.Name, FamiliaColocada)
...
# Documentos vinculados
for linked in DocumentoSeleccionado:
    doc_link = linked.GetLinkDocument()
    if search not in linked.Name:
        continue
    
    collector_CD = FilteredElementCollector(doc_link).OfCategory(BuiltInCategory.OST_CommunicationDevices).WhereElementIsNotElementType().ToElements()
    ...
    for elemento in collector_CD:
        ...
        try:
            if NombreDeFamilia == 'TV_wall':
                AlturaTV = elemento.get_Symbol().LookupParameter('alto').AsDouble()
                Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z - (AlturaTV / 2 + UnitUtils.ConvertToInternalUnits(100, Document.GetUnits(doc).GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())))
                
                # Colocar Familia
                collector_EF = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalFixtures).WhereElementIsElementType().ToElements()
                familia = next((i for i in collector_EF if i.Family.Name == 'TR_wall'), None)
                if not familia:
                    raise ValueError(f"Family 'TR_wall' not found.")
                ...
                if not familia.IsActive:
                    familia.Activate()
                FamiliaColocada = doc.Create.NewFamilyInstance(Punto2, familia, Nivel, StructuralType.NonStructural)
                TransactionManager.Instance.ForceCloseTransaction()
                
                # Girar Elemento
                rotation = elemento.FacingOrientation
                fpoint = FamiliaColocada.Location.Point
                rAxis = Line.CreateBound(fpoint, XYZ(fpoint.X, fpoint.Y, fpoint.Z + 1))
                angle = rotation.AngleTo(XYZ(0, 1, 0))
                
                TransactionManager.Instance.EnsureInTransaction(doc)
                ElementTransformUtils.RotateElement(doc, FamiliaColocada.Id, rAxis, -angle)
                TransactionManager.Instance.TransactionTaskDone()
                
                # Colocar Valor Eléctrico
                power_map = {
                    '32"': 50,
                    '40"': 60,
                    '50"': 65,
                ...
                power = power_map.get(elemento.Name)
                if power is None:
                    raise ValueError(f"Power for type '{elemento.Name}' not found.")
                
                proj_ut = Document.GetUnits(doc).GetFormatOptions(SpecTypeId.ElectricalPotential).GetUnitTypeId()
                power_internal = UnitUtils.ConvertToInternalUnits(power, proj_ut)
                
                TransactionManager.Instance.EnsureInTransaction(doc)
                FamiliaColocada.LookupParameter('power').Set(power_internal)
                TransactionManager.Instance.ForceCloseTransaction()
                
                NombresTV.append(elemento.Name)
                ValoresElectricidad.append(FamiliaColocada.LookupParameter('power').AsValueString())
                ...
                # Colocar Familia
                collector_EF = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalFixtures).WhereElementIsElementType().ToElements()
                familia = next((i for i in collector_EF if i.Family.Name == 'TR_ceiling'), None)
                if not familia:
                    raise ValueError(f"Family 'TR_ceiling' not found.")
    ...
    output = list(zip(NombresTV, ValoresElectricidad))
    break
...
# Añadiendo referencias necesarias para trabajar con Revit y Dynamo
clr.AddReference('ProtoGeometry')
from Autodesk.DesignScript.Geometry import *
clr.AddReference('RevitAPI')
from Autodesk.Revit.DB import *
...
# Document Manager
# Obteniendo el documento actual de Revit y la aplicacion de la interfaz de usuario
doc = DocumentManager.Instance.CurrentDBDocument
uiapp = DocumentManager.Instance.CurrentUIApplication
app = uiapp.Application
uidoc = uiapp.ActiveUIDocument

# Aliases para simplificar el uso de clases
from Autodesk.Revit.DB import FilteredElementCollector as Fec
from Autodesk.Revit.DB import BuiltInCategory as Bic

# Diccionario con la relacion de potencias electricas segun el tamaño de las pulgadas de los televisores
Relacion_Potencia_Pulgadas = {'32"': 50, '40"': 60, '50"': 65, '55"': 70, '65"': 80,}

# Revision de documentos vinculados
# Si el documento vinculado no es "modeloAV.rvt", se pasa al siguiente archivo vinculado
for DocumentoSeleccionado in FilteredElementCollector(doc).OfClass(RevitLinkInstance).ToElements():
    Link_Documento_Seleccionado_ = DocumentoSeleccionado.GetLinkDocument()
    if 'modeloAV.rvt' not in DocumentoSeleccionado.Name:
        continue

    # Creacion de un colector para todos los dispositivos de comunicacion en el documento vinculado
    collector_CD = FilteredElementCollector(Link_Documento_Seleccionado_).OfCategory(BuiltInCategory.OST_CommunicationDevices).WhereElementIsNotElementType().ToElements()
    for elemento in collector_CD:
        NombreDeFamilia = elemento.get_Symbol().FamilyName
        Punto_1 = elemento.Location.Point
        ...
        try:
            if NombreDeFamilia == 'TV_wall':
                # Si el dispositivo es un televisor de pared, se calcula la ubicacion del toma de corriente
                AlturaTV = elemento.get_Symbol().LookupParameter('alto').AsDouble()
                Punto1 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z - (AlturaTV / 2 + UnitUtils.ConvertToInternalUnits(100, Document.GetUnits(doc).GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())))
                
                # Colector de elementos de tipo fixture electrico en el documento actual
                collector_EF = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalFixtures).WhereElementIsElementType().ToElements()
                familia = None
                for i in collector_EF:
                    if i.Family.Name == 'TR_wall':
                        familia = i
                        break
                Nivel = doc.ActiveView.GenLevel
                
                # Inicia una transaccion para crear una instancia de la familia en el punto calculado
                TransactionManager.Instance.EnsureInTransaction(doc)
                if not familia.IsActive:
                    familia.Activate()
                FamiliaColocada = doc.Create.NewFamilyInstance(Punto1, familia, Nivel, StructuralType.NonStructural)
                TransactionManager.Instance.ForceCloseTransaction()
                
                # Calcula la rotacion necesaria para alinear la familia con el televisor
                OrientacionDeLaCara = elemento.FacingOrientation
                LP_FamiliaColocada = FamiliaColocada.Location.Point
                Vector = Line.CreateBound(LP_FamiliaColocada, XYZ(LP_FamiliaColocada.X, LP_FamiliaColocada.Y, LP_FamiliaColocada.Z + 1))
                AnguloDeRotacion = OrientacionDeLaCara.AngleTo(XYZ(0, 1, 0))
                
                # Aplica la rotacion a la familia colocada
                TransactionManager.Instance.EnsureInTransaction(doc)
                ElementTransformUtils.RotateElement(doc, FamiliaColocada.Id, Vector, -AnguloDeRotacion)
                TransactionManager.Instance.TransactionTaskDone()

                # Asigna la potencia correspondiente al televisor
                Potencia = Relacion_Potencia_Pulgadas.get(elemento.Name)
                Unidades_Potencia = Document.GetUnits(doc).GetFormatOptions(SpecTypeId.ElectricalPotential).GetUnitTypeId()
                Potencia_UnidadesInternas = UnitUtils.ConvertToInternalUnits(Potencia, Unidades_Potencia)
                
                # Inicia una transaccion para establecer el parametro de potencia
                TransactionManager.Instance.EnsureInTransaction(doc)
                FamiliaColocada.LookupParameter('power').Set(Potencia_UnidadesInternas)
                TransactionManager.Instance.ForceCloseTransaction()
                
            elif NombreDeFamilia == 'TV_ceiling':
                # Si el dispositivo es un televisor de techo, se calcula la ubicacion de la toma de corriente
                AlturaSoporteTV = elemento.LookupParameter('alto_soporte').AsDouble()
                Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z + AlturaSoporteTV)
...
# Función auxiliar para crear, colocar, rotar y asignar potencia a una familia
def crear_y_colocar_familia(elemento, nombre_familia_buscar, punto, potencia_parametro):
    # Buscar la familia de tipo fixture eléctrico en el documento actual
    collector_EF = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_ElectricalFixtures).WhereElementIsElementType().ToElements()
    familia = next((i for i in collector_EF if i.Family.Name == nombre_familia_buscar), None)
    
    # Si no se encuentra la familia, salir
    if not familia:
        return None
    
    # Obtener el nivel de la vista activa
    Nivel = doc.ActiveView.GenLevel
    
    # Iniciar una transacción para crear una instancia de la familia en el punto especificado
    TransactionManager.Instance.EnsureInTransaction(doc)
    if not familia.IsActive:
        familia.Activate()
    FamiliaColocada = doc.Create.NewFamilyInstance(punto, familia, Nivel, StructuralType.NonStructural)
    TransactionManager.Instance.ForceCloseTransaction()
    
    # Obtener la orientación de la cara del elemento
    OrientacionDeLaCara = elemento.FacingOrientation
    LP_FamiliaColocada = FamiliaColocada.Location.Point
    Vector = Line.CreateBound(LP_FamiliaColocada, XYZ(LP_FamiliaColocada.X, LP_FamiliaColocada.Y, LP_FamiliaColocada.Z + 1))
    AnguloDeRotacion = OrientacionDeLaCara.AngleTo(XYZ(0, 1, 0))
    
    # Aplicar la rotación a la familia colocada
    TransactionManager.Instance.EnsureInTransaction(doc)
    ElementTransformUtils.RotateElement(doc, FamiliaColocada.Id, Vector, -AnguloDeRotacion)
    TransactionManager.Instance.TransactionTaskDone()
    
    # Asignar la potencia correspondiente al televisor
    Potencia = Relacion_Potencia_Pulgadas.get(elemento.Name)
    Unidades_Potencia = Document.GetUnits(doc).GetFormatOptions(SpecTypeId.ElectricalPotential).GetUnitTypeId()
    Potencia_UnidadesInternas = UnitUtils.ConvertToInternalUnits(Potencia, Unidades_Potencia)
    
    # Iniciar una transacción para establecer el parámetro de potencia
    TransactionManager.Instance.EnsureInTransaction(doc)
    FamiliaColocada.LookupParameter(potencia_parametro).Set(Potencia_UnidadesInternas)
    TransactionManager.Instance.ForceCloseTransaction()
    
    return FamiliaColocada

# Iterar sobre todos los documentos vinculados
for DocumentoSeleccionado in FilteredElementCollector(doc).OfClass(RevitLinkInstance).ToElements():
    Link_Documento_Seleccionado_ = DocumentoSeleccionado.GetLinkDocument()
    
    # Si el documento vinculado no es "modeloAV.rvt", continuar con el siguiente archivo vinculado
    if 'modeloAV.rvt' not in DocumentoSeleccionado.Name:
        continue

    # Crear un colector para todos los dispositivos de comunicación en el documento vinculado
    collector_CD = FilteredElementCollector(Link_Documento_Seleccionado_).OfCategory(BuiltInCategory.OST_CommunicationDevices).WhereElementIsNotElementType().ToElements()
    
    # Iterar sobre cada elemento en el colector
    for elemento in collector_CD:
        NombreDeFamilia = elemento.get_Symbol().FamilyName
        Punto_1 = elemento.Location.Point
        ...
        try:
            if NombreDeFamilia == 'TV_wall':
                # Si el dispositivo es un televisor de pared, calcular la ubicación del toma de corriente
                AlturaTV = elemento.get_Symbol().LookupParameter('alto').AsDouble()
                Punto1 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z - (AlturaTV / 2 + UnitUtils.ConvertToInternalUnits(100, Document.GetUnits(doc).GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())))
                
                # Crear y colocar la familia en el punto calculado
                crear_y_colocar_familia(elemento, 'TR_wall', Punto1, 'power')
                
            elif NombreDeFamilia == 'TV_ceiling':
                # Si el dispositivo es un televisor de techo, calcular la ubicación del toma de corriente
                AlturaSoporteTV = elemento.LookupParameter('alto_soporte').AsDouble()
                Punto2 = XYZ(Punto_1.X, Punto_1.Y, Punto_1.Z + AlturaSoporteTV)
                
                # Crear y colocar la familia en el punto calculado
                crear_y_colocar_familia(elemento, 'TR_ceiling', Punto2, 'power')
            
            else:
                # Si el dispositivo no es un televisor de pared o techo, imprimir un mensaje
                print("No existe las familias del tipo TV_wall o TV_ceiling")
...
# Diccionario con la relación de potencias eléctricas según el tamaño de las pulgadas de los televisores
RELACION_POTENCIA_PULGADAS = {'32"': 50, '40"': 60, '50"': 65, '55"': 70, '65"': 80}
...
def crear_y_colocar_familia(elemento, nombre_familia_buscar, punto, potencia_parametro):
    ...
    # Crear una instancia de la familia en el punto especificado
    with Transaction(doc, "Crear y Colocar Familia") as t:
        t.Start()
        if not familia.IsActive:
            familia.Activate()
        familia_colocada = doc.Create.NewFamilyInstance(punto, familia, nivel, StructuralType.NonStructural)
        t.Commit()
    
    # Obtener la orientación de la cara del elemento
    orientacion_de_la_cara = elemento.FacingOrientation
    lp_familia_colocada = familia_colocada.Location.Point
    vector = Line.CreateBound(lp_familia_colocada, XYZ(lp_familia_colocada.X, lp_familia_colocada.Y, lp_familia_colocada.Z + 1))
    angulo_de_rotacion = orientacion_de_la_cara.AngleTo(XYZ(0, 1, 0))
    
    # Aplicar la rotación a la familia colocada
    with Transaction(doc, "Rotar Familia") as t:
        t.Start()
        ElementTransformUtils.RotateElement(doc, familia_colocada.Id, vector, -angulo_de_rotacion)
        t.Commit()
    
    # Asignar la potencia correspondiente al televisor
    potencia = RELACION_POTENCIA_PULGADAS.get(elemento.Name)
    if potencia:
        unidades_potencia = doc.GetUnits().GetFormatOptions(SpecTypeId.ElectricalPotential).GetUnitTypeId()
        potencia_unidades_internas = UnitUtils.ConvertToInternalUnits(potencia, unidades_potencia)

        with Transaction(doc, "Asignar Potencia") as t:
            t.Start()
            familia_colocada.LookupParameter(potencia_parametro).Set(potencia_unidades_internas)
            t.Commit()
    
    return familia_colocada

def procesar_documento_vinculado(documento_vinculado):
    link_documento_seleccionado = documento_vinculado.GetLinkDocument()
    
    if 'modeloAV.rvt' not in documento_vinculado.Name:
        return

    collector_CD = FilteredElementCollector(link_documento_seleccionado).OfCategory(BuiltInCategory.OST_CommunicationDevices).WhereElementIsNotElementType().ToElements()
    
    for elemento in collector_CD:
        nombre_de_familia = elemento.get_Symbol().FamilyName
        punto_1 = elemento.Location.Point
        
        try:
            if nombre_de_familia == 'TV_wall':
                altura_tv = elemento.get_Symbol().LookupParameter('alto').AsDouble()
                punto1 = XYZ(punto_1.X, punto_1.Y, punto_1.Z - (altura_tv / 2 + UnitUtils.ConvertToInternalUnits(100, doc.GetUnits().GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())))
                crear_y_colocar_familia(elemento, 'TR_wall', punto1, 'power')
                
            elif nombre_de_familia == 'TV_ceiling':
                altura_soporte_tv = elemento.LookupParameter('alto_soporte').AsDouble()
                punto2 = XYZ(punto_1.X, punto_1.Y, punto_1.Z + altura_soporte_tv)
                crear_y_colocar_familia(elemento, 'TR_ceiling', punto2, 'power')
...
# Iterar sobre todos los documentos vinculados
for documento_seleccionado in FilteredElementCollector(doc).OfClass(RevitLinkInstance).ToElements():
    procesar_documento_vinculado(documento_seleccionado)

# ----------------------------------------------------------------------------------

# Este es el que estoy arreglando - Cambiar por ultima version
...
def crear_y_colocar_familia(elemento, nombre_familia_buscar, punto, potencia_parametro):
    ...
    # Crear una instancia de la familia en el punto especificado
    with Transaction(doc, "Crear y Colocar Familia") as Transaccion_ColocarFamilias:
        Transaccion_ColocarFamilias.Start()
        if not familia.IsActive:
            familia.Activate()
        familia_colocada = doc.Create.NewFamilyInstance(punto, familia, nivel, StructuralType.NonStructural)
        Transaccion_ColocarFamilias.Commit()
    ...
    # Aplicar la rotación a la familia colocada
    with Transaction(doc, "Rotar Familia") as Transaccion_RotarFamilias:
        Transaccion_RotarFamilias.Start()
        ElementTransformUtils.RotateElement(doc, familia_colocada.Id, vector, -angulo_de_rotacion)
        Transaccion_RotarFamilias.Commit()
    ...
    if potencia:
        ...
        with Transaction(doc, "Asignar Potencia") as Transaccion_AsignarPotencia:
            Transaccion_AsignarPotencia.Start()
            familia_colocada.LookupParameter(potencia_parametro).Set(potencia_unidades_internas)
            Transaccion_AsignarPotencia.Commit()
...
def procesar_documento_vinculado(documento_vinculado):
    ...
    for elemento in collector_CD:
        ...
        try:
            if nombre_de_familia == 'TV_wall' and FamiliaIngresada == 'TV_wall':
                altura_tv = elemento.get_Symbol().LookupParameter('alto').AsDouble()
                punto1 = XYZ(punto_1.X, punto_1.Y, punto_1.Z - (altura_tv / 2 + UnitUtils.ConvertToInternalUnits(100, doc.GetUnits().GetFormatOptions(SpecTypeId.Length).GetUnitTypeId())))
                crear_y_colocar_familia(elemento, 'TR_wall', punto1, 'power')
                
            elif nombre_de_familia == "TV_ceiling" and FamiliaIngresada == 'TV_ceiling':
                altura_soporte_tv = elemento.LookupParameter('alto_soporte').AsDouble()
                punto2 = XYZ(punto_1.X, punto_1.Y, punto_1.Z + altura_soporte_tv)
                crear_y_colocar_familia(elemento, 'TR_ceiling', punto2, 'power')
...
Lista = []  # Inicializa 'Lista' como una lista vacía

def crear_nivel(elevacion):
    
    # Iniciar la transacción
    TransactionManager.Instance.EnsureInTransaction(doc)
    
    try:
        # Crear un nuevo nivel en la elevación especificada
        nuevo_nivel = Level.Create(doc, elevacion)
        ...
        TransactionManager.Instance.ForceCloseTransaction()
        Lista.append("error",elevacion)
        return None

# Iterar sobre las dos listas al mismo tiempo
for altura in zip(Alturas):
    nivel_creado = crear_nivel(altura)  # Crea el nivel
    if nivel_creado is not None:  # Verificar si se creó el nivel con éxito
        Lista.append(nivel_creado)  # Agrega el nivel creado a la lista
